...
program:
    create_global_scope statements leave_global_scope {
        auto&& root_scope = last::node::create(last::node::Scope(std::move($2)));
        program = last::AST(std::move(root_scope));
    }
    ;
...
assignment:
    ...
    | NAME AS error {
        ErrorHandler::showError(@3, "expected expression after assignment");
        YYABORT;
    }
    ;
...
print_expression:
    ...
    | PRINT error {
        ErrorHandler::showError(@2, "expected expressions after 'print'");
        YYABORT;
    }
    ;
...
while_statement:
    WHILE LCIB expression RCIB LCUB scope RCUB {
        auto&& w = last::node::While(std::move($3), std::move($6));
        $$ = last::node::create(std::move(w));
    }
    | WHILE LCIB expression RCIB one_stmt_scope {
        auto&& w = last::node::While(std::move($3), std::move($5));
        $$ = last::node::create(std::move(w));
    }
    | WHILE LCIB error RCIB LCUB scope RCUB { ErrorHandler::showError(@3, "expected condition in while"); YYABORT; }
    | WHILE LCIB expression error LCUB scope RCUB { ErrorHandler::showError(@4, "expected ')' after while condition"); YYABORT; }
    | WHILE LCIB expression RCIB error { ErrorHandler::showError(@5, "expected scope after 'while (...)'"); YYABORT; }
...
if_statement:
    ...
    | IF LCIB error RCIB LCUB scope RCUB { ErrorHandler::showError(@3, "expected condition in if"); YYABORT; }
    | IF LCIB expression error LCUB scope RCUB { ErrorHandler::showError(@4, "expected ')' after if"); YYABORT; }
    | IF LCIB expression RCIB error { ErrorHandler::showError(@5, "expected scope after if"); YYABORT; }
    | IF error { ErrorHandler::showError(@2, "expected '(' after if"); YYABORT; }
    ;
...
else_statement:
    ...
    | ELSE error { ErrorHandler::showError(@2, "expected scope after 'else'"); YYABORT; }
    ;
...
primary_expression:
    ...
    | NAME {
        if (name_table.is_not_force_declare($1)) {
            ErrorHandler::showError(@1, "using undeclarated variable: " + $1);
            YYABORT;
        }
        $$ = last::node::create(last::node::Variable(std::move($1)));
    }
