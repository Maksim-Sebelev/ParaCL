...
# Синтаксис:
В отличие от С и на радость питонистам все происходит в global-scope. Тип переменных всегда int. Есть поддержка while, if/else if/else. Доступным комментарии: # - для однострочных, /* ... */ - для многострочных. Существует оператор вывода print и оператор ввода ?.\
Примеры програм можно посмотреть в папке `tests/e2e/dat/`.
