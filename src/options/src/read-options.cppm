...
)
{
  ...
  explain << BOLD "paracl: " RESET ERROR "error:" RESET " " BOLD << msg
          << RESET "\n";
...
public:
  ...
  {
    auto&& explain = "Unsupported file format: " +
                     file.string() +
                     ". Expected '.cl' file as input.";
    msg_           = show_error(explain);
  }
};
...
{
public:
  unsupported_optimize_level_error(
      std::string_view optimize_level
  )
  {
    auto&& explain = "Unsupported optimize level '" +
                     std::string(optimize_level) +
                     "'. Supported values are 0, 1, 2 or 3";
    msg_           = show_error(explain);
  }
};
...
Suppoted options)"
  ...
  if (not vm.count("input")) throw error::no_input_files_error();
  ...
  if (vm.count("optimize-level"))
  {
    ...
    if (opt_level == "0") options.optimize_level = OptimizeLevel::O0;
    else if (opt_level == "1") options.optimize_level = OptimizeLevel::O1;
    else if (opt_level == "2") options.optimize_level = OptimizeLevel::O2;
    else if (opt_level == "3") options.optimize_level = OptimizeLevel::O3;
    else throw error::unsupported_optimize_level_error(opt_level);
  }
