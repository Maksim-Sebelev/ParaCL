...
Главным классом всего проекта является `frontend::ast::BasicNode`. Он реализует паттерн type erasure для нод в проекте (здесь нет привязки к слову "нода", он может стирать любой тип). Как Вам несомненно известно, суть паттерна type erasure есть совмещение двух идей, назовем их "внешними виртуальными функциями" и "хранением разных объектов в одном контейнере с семантикой значения, а не указателей".
Однако сами ноды дерева содержат поля типа frontend::ast::BasicNode (или зависящего от frontend::ast::BasicNode), поэтому возникает проблема с классическим подходом реализации методов работы с нодами через перегрузку внешних функцию, а именно:
так как сам frontend::ast::BasicNode становится полноценным участником дерева, то он должен реализовывать все методы работы с ним, которые нужны при работе с деревом. Но конечно серьезной ошибкой проектирования было бы реализовать функцию в духе 'void interpret(frontend::ast::AST const& ast)' в библиотеке самого ast. Поэтому классическая реализация type erasure здесь неприменима.
...
Причина по которой шаблонные виртуальные функции невозожны в С++ - это то, что базовый и наследуемый классы могут находиться с разных единицах трансляции, потому неясно что и где инстанцировать. Но в паттерн type erasure скрывает наследование как деталь реализации одного класса, потому здесь можно попытаться изобрести эти самые шаблонные виртуальные функции. В классе `frontend::ast::BasicNode` есть шаблонный подкласс `template <typename... Singantures> frontend::ast::BasicNode::Actions`, а в нем статический метод `template <typename NodeT> frontend::ast::BasicNode create(NodeT&& node)`. Данный метод заменяет конструктор класса. Шаблонный параметр самого метода есть тип, который стирает BasicNode, аргумент - соостветсвующий объект. Шаблонные параметры подкласса Action есть сигнатуры "виртуальных функций", которые должна поддерживать эта нода. Это стоит рассматривать как просьбу к компилятору инстанциовать такие-то "специализации шаблонной виртуальной функции visit" для этого этой ноды.\
Например:
...
auto&& left = /* ... */;
auto&& right = /* ... */;
auto&& bin_op = frontend::ast::node::BinaryOperator{frontend::ast::node::BinaryOperator::PLUS, std::move(left), std::move(right)};
auto&& node = frontend::ast::node::BasicNode::Actions<dumpable, calculable>::create(std::move(bin_op));
...
## Как реализовать свой метод работы с frontend::ast::node::BasicNode?

В модуле, отвечающем за BasicNode, есть шаблонная функция:

```cpp
template <typename NodeT, typename ReturnT, typename... Args>
ReturnT frontend::ast::node::visit_specializations::visit(NodeT const &, Args...)
{
...
SET_NODE_FUNCTIONALITY(Print, dumpale)
...
```cpp
template <>
frontend::ast::node::BasicNode frontend::ast::node::create(NodeT node)
{
    return frontend::ast::node::BasicNode::Actions<__VA_ARGS__>::create(std::move(node));
}
```
...
SET_NODE_FUNCTIONALITY(Scope           , __VA_ARGS__)                                                                                   \
SET_NODE_FUNCTIONALITY(Print           , __VA_ARGS__)                                                                                   \
SET_NODE_FUNCTIONALITY(Scan            , __VA_ARGS__)                                                                                   \
SET_NODE_FUNCTIONALITY(UnaryOperator   , __VA_ARGS__)                                                                                   \
SET_NODE_FUNCTIONALITY(BinaryOperator  , __VA_ARGS__)                                                                                   \
SET_NODE_FUNCTIONALITY(If              , __VA_ARGS__)                                                                                   \
SET_NODE_FUNCTIONALITY(Else            , __VA_ARGS__)                                                                                   \
SET_NODE_FUNCTIONALITY(Condition       , __VA_ARGS__)                                                                                   \
SET_NODE_FUNCTIONALITY(While           , __VA_ARGS__)                                                                                   \
SET_NODE_FUNCTIONALITY(NumberLiteral   , __VA_ARGS__)                                                                                   \
SET_NODE_FUNCTIONALITY(StringLiteral   , __VA_ARGS__)                                                                                   \
SET_NODE_FUNCTIONALITY(Variable        , __VA_ARGS__)
```
...
```cpp
void frontend::ast::write(frontend::ast::AST const & ast, std::filesystem::path const &file);
```
...
```cpp
frontend::ast::AST frontend::ast::read(std::filesystem::path const & ast_txt);
```
...
```cpp
void dump(frontend::ast::AST const & ast, std::filesystem::path const & dot_file, std::filesystem::path const & img_file = "");
```
...
bool frontend::ast::node::BasicNode::supports();
```
...
```cpp
template <typename T>
bool frontend::ast::node::BasicNode::is_a();
```
...
Хочется в конце подвести небольшой итог, по поводу данной библиотеки, вспомнив преимущества и недостатки:

**Преимущества:**

1. независимость нод от методов работы с ними -> SRP
2. ноды полностью самодостаточны и не требуют изменений при добавлении/расширении функционала -> OCP
3. (2.2) ноды не зависят от грамматики языка -> нет необходимости в них что-либо менять при изменении грамматики
4. простое добавление новых нод (без необходимости наследования)

**Недостатки:**

1) зависимость от поддерживаемого функционала на этапе конструирования -> статический шаблонный метод create
2) (1.2) необхидость ручного указания поддерживаемого функционала через специализацию create
3) (1.2.1) 2 => макросы для специализаций create
4) (1.3) подверженность ошибкам по типу неуказания нужного функуционала в create или отсуствия нужной специализации visit, которые крайне трудно искать.
5) (1.4) трудность передачи ast между модулями, из-за необходимости при конструировании знать поддерживаемый функционал
6) (1.5) 5 => необходимость функции read
