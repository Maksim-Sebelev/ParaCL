...
export namespace visit_specializations
{
...
)
{
  static_assert(
      false,
      "using not specialized visit function. "
...
private:
  struct IBaseNode
  {
    ...
    virtual
    std::any
    invoke_(std::type_info const& sig, std::any* args) const = 0;
    virtual
    bool
    supports_signature_(std::type_info const& sig) const = 0;
  ...
  template <typename NodeT, typename... Signatures>
    requires(std::is_function_v<Signatures> and ...)
  struct NodeImpl final : public IBaseNode
  {
    ...
    struct Invoker<ReturnT(Args...)>
    {
      ...
      )
      {
        if constexpr (std::is_reference_v<T>)
        {
          ...
          auto&& wrapper =
              std::any_cast<WrappedType&>(const_cast<std::any&>(arg));
          return wrapper.get();
        }
        else return std::any_cast<T>(arg);
      }
      ...
      std::any
      call_impl_(
          NodeT const& data, std::any* args, std::index_sequence<Is...>
      )
      {
        if constexpr (std::is_void_v<ReturnT>)
        {
          visit_specializations::template visit<NodeT, ReturnT, Args...>(
              data, unwrap_arg_<Args>(args[Is])...
          );
          return std::any{};
        }
        else
    ...
    template <size_t... Is>
    bool
    supports_signature_impl_(
        std::type_info const& sig, std::index_sequence<Is...>
    ) const
    {
    ...
    std::any
    invoke_impl_(
        std::type_info const& sig, std::any* args, std::index_sequence<Is...>
    ) const
    {
      std::any result;
      auto&&   found = false;
      ...
      throw std::runtime_error(
          ...
          sig.name() +
          " was not register for this node. supported:: " +
          supported
      );
    }
  ...
  public:
    ...
    std::any
    invoke_(
        std::type_info const& sig, std::any* args
    ) const override
    {
    ...
    bool
    supports_signature_(
        std::type_info const& sig
    ) const override
    {
      return supports_signature_impl_(
          sig, std::index_sequence_for<Signatures...>{}
      );
    }
  };
  ...
  {
    if constexpr (std::is_lvalue_reference_v<T>) return std::ref(arg);
    else return std::any(std::forward<T>(arg));
  }
...
public:
  ...
  but ir_translator cannot do this for functions signature, because it`s not
  using clearly
  */
  ...
  struct Actions
  {
    ...
    {
      auto&& impl = std::make_unique<NodeImpl<NodeT, Signatures...>>(
  ...
  if signature is no supported by the node - function will throw
  std::runtime_error
  */
  template <typename ReturnT, typename... Args>
  friend
  ReturnT
  visit(
      BasicNode const& node, Args... args
  )
  {
    if (not node.self_)
      throw std::runtime_error(
          "visite node, which is not constructible (node.self_ is nullptr)"
      );
    ...
    if (not node.self_->supports_signature_(typeid(SignatureT)))
    {
      throw std::runtime_error(
          std::string(
              "This node '" +
              std::string(node.self_->type_().name()) +
              "' dont supports this function: "
          ) +
          typeid(SignatureT).name()
      );
    }

    std::any arg_array[] = {pack_arg_(std::forward<Args>(args))...};
    auto&&   result      = node.self_->invoke_(typeid(SignatureT), arg_array);
  ...
  */
  ...
  BasicNode&
  operator =(BasicNode&&) noexcept = default;
  ...
  supports() const
  {
    return (self_) and (self_->supports_signature_(typeid(Signatures)) && ...);
  }
  ...
  /* implicit */
  operator bool() const noexcept { return static_cast<bool>(self_); }
  ...
  operator T const&() const
  {
    if (not is_a<T>())
      throw std::runtime_error(
          "Bad frontend::ast::node::BasicNode cast: real type: '" +
          std::string(self_->type_().name()) +
          "', casted type: '" +
          typeid(T).name() +
          std::string("'")
      );
