...
llvm::Value*
create_null_Int32(
    llvmIrTranslatorContext& context
)
{
...
llvm::Value*
convert_to_Int1(
    llvmIrTranslatorContext& context, llvm::Value* value
)
{
  assert(value);
...
llvm::Value*
convert_Int1_to_Int32(
    llvmIrTranslatorContext& context, llvm::Value* value,
    std::string_view description = "__convertInt1ToInt32"
)
{
  assert(value);
  return context.builder.CreateZExt(
      value, context.builder.getInt32Ty(), description
  );
}
...
bool
is_semicolon_or_empty_scope(
    ast::node::BasicNode const& node
)
{
  return node.is_a<ast::node::Semicolon>() or
         (node.is_a<ast::node::Scope>() and
          static_cast<ast::node::Scope const&>(node).empty());
}
...
generate_if_statement(
    BasicNode const& node, llvmIrTranslatorContext& context,
    llvm::BasicBlock* self_condition, llvm::BasicBlock* self_body,
    llvm::BasicBlock* next, llvm::BasicBlock* end
)
{
  return visit<void, llvmIrTranslatorContext&, llvm::BasicBlock*>(
      node, context, self_condition, self_body, next, end
  );
}
...
)
{
  ...
  std::ignore = visit<NumberLiteral, llvm::Value*, llvmIrTranslatorContext&>(
      node, context
  );
}
...
llvm::Value*
visit(
    StringLiteral const& node, llvmIrTranslatorContext& context
)
{
...
{
  warning::expression_result_unused(node.location());
  std::ignore = visit<StringLiteral, llvm::Value*, llvmIrTranslatorContext&>(
      node, context
  );
}
...
llvm::Value*
visit(
    Variable const& node, llvmIrTranslatorContext& context
)
{
  auto&& variable = context.nametable.get(node.name());
  if (not variable) throw error::using_undeclarated_variable(node);

  if (context.nametable.is_function(node.name()))
    throw error::using_function_as_int(
        node, context.nametable.get_info(node.name()).declaration_node
    );

  if (not context.nametable.is_visible_from(
          node.name(), context.current_block->getParent()
      ))
    throw error::using_variable_from_parent_function_scope(
        node, context.nametable.get_info(node.name()).declaration_node
    );
...
)
{
  warning::expression_result_unused(node.location());
  std::ignore =
      visit<Variable, llvm::Value*, llvmIrTranslatorContext&>(node, context);
}
...
llvm::Value*
visit(
    Scan const& node, llvmIrTranslatorContext& context
)
{
  static auto&& fmt =
      context.builder.CreateGlobalStringPtr("%d", "__scanfFormat");

  auto&& scanf_res_alloca = context.builder.CreateAlloca(
      context.builder.getInt32Ty(), nullptr, "__scanfResAlloca"
  );
  auto&& scanf_args = std::vector<llvm::Value*>{fmt, scanf_res_alloca};

  context.builder.CreateCall(
      context.libc_standart_functions.libc_scanf(),
      scanf_args,
      "__scanfExitCode"
  );

  return context.builder.CreateLoad(
      context.builder.getInt32Ty(), scanf_res_alloca, "__scanfRes"
  );
}
...
)
{
  warning::expression_result_unused(node.location());
  std::ignore =
      visit<Scan, llvm::Value*, llvmIrTranslatorContext&>(node, context);
}
...
llvm::Value*
visit(
    UnaryOperator const& node, llvmIrTranslatorContext& context
)
{
  ...
  switch (node.type())
  {
    case UnaryOperator::PLUS: return arg;
    ...
    case UnaryOperator::NOT:
    {
      auto&& zero = frontend::llvm_ir_translator::create_null_Int32(context);
      auto&& cmp =
          context.builder.CreateICmpEQ(arg, zero, "__tmpUnaryNotCmpWithZero");
      return frontend::llvm_ir_translator::convert_Int1_to_Int32(
          context, cmp, "__unaryNot"
      );
    }
    default: break;
  }
...
{
  warning::expression_result_unused(node.location());
  std::ignore = visit<UnaryOperator, llvm::Value*, llvmIrTranslatorContext&>(
      node, context
  );
}
...
llvm::Value*
visit(
    BinaryOperator const& node, llvmIrTranslatorContext& context
)
{
  auto&& type = node.type();
  ...
  if (type == BinaryOperator::ASGN)
  {
    ...
    context.nametable.set(
        variable.name(), right, variable, context.current_scope_status
    );
    return right;
  }
  ...
  switch (type)
  {
    case BinaryOperator::ADD:
      return context.builder.CreateAdd(left, right, "__add");
    case BinaryOperator::SUB:
      return context.builder.CreateSub(left, right, "__sub");
    case BinaryOperator::MUL:
      return context.builder.CreateMul(left, right, "__mul");
    case BinaryOperator::DIV:
      return context.builder.CreateSDiv(left, right, "__div");
    case BinaryOperator::REM:
      return context.builder.CreateSRem(left, right, "__rem");
    case BinaryOperator::AND:
    {
      auto&& left_Int1 =
          frontend::llvm_ir_translator::convert_to_Int1(context, left);
      auto&& right_Int1 =
          frontend::llvm_ir_translator::convert_to_Int1(context, right);

      auto&& and_result =
          context.builder.CreateAnd(left_Int1, right_Int1, "__andBool");
      return frontend::llvm_ir_translator::convert_Int1_to_Int32(
          context, and_result, "__and"
      );
    }
    case BinaryOperator::OR:
    {
      ...
      auto&& or_result =
          context.builder.CreateOr(left_Int1, right_Int1, "__orBool");
      return frontend::llvm_ir_translator::convert_Int1_to_Int32(
          context, or_result, "__or"
      );
    }
  ...
  switch (type)
  {
    case BinaryOperator::ADDASGN:
      right = context.builder.CreateAdd(value, right, "__addAsgnResult");
      break;
    case BinaryOperator::SUBASGN:
      right = context.builder.CreateSub(value, right, "__subAsgnResult");
      break;
    case BinaryOperator::MULASGN:
      right = context.builder.CreateMul(value, right, "__mulAsgnResult");
      break;
    case BinaryOperator::DIVASGN:
      right = context.builder.CreateSDiv(value, right, "__divAsgnResult");
      break;
    case BinaryOperator::REMASGN:
      right = context.builder.CreateSRem(value, right, "__remAsgnResult");
      break;
    default:
#if defined(NDEBUG)
      __builtin_unreachable();
...
)
{
  ...
  std::ignore = visit<BinaryOperator, llvm::Value*, llvmIrTranslatorContext&>(
      node, context
  );
}
...
llvm::Value*
visit(
    Print const& node, llvmIrTranslatorContext& context
)
{
  ...
  if (fmt_str == "%d\n")
  {
    static auto&& fmt_percent_d =
        context.builder.CreateGlobalStringPtr(fmt_str, "__printfFormat");
  ...
  return context.builder.CreateCall(
      context.libc_standart_functions.libc_printf(),
      printf_args,
      "__printfExitCode"
  );
}
...
{
  std::ignore =
      visit<Print, llvm::Value*, llvmIrTranslatorContext&>(node, context);
}
...
void
visit(
    While const& node, llvmIrTranslatorContext& context
)
{
  auto&& current_func = context.current_block->getParent();

  auto&& cond_block =
      llvm::BasicBlock::Create(context.context, "while", current_func);
  auto&& body_block =
      llvm::BasicBlock::Create(context.context, "do", current_func);
  auto&& end_block =
  ...
  auto&& cond_i1  = context.builder.CreateICmpNE(
      cond_val,
      frontend::llvm_ir_translator::create_null_Int32(context),
      "__whileCondition"
  );
...
void
visit(
    If const& node, llvmIrTranslatorContext& context,
    llvm::BasicBlock* self_condition, llvm::BasicBlock* self_body,
    llvm::BasicBlock* next, llvm::BasicBlock* end
)
{
  ...
  auto&& if_condition_bool = context.builder.CreateICmpNE(
      if_condition,
      frontend::llvm_ir_translator::create_null_Int32(context),
      "__condition"
  );
  ...
  if (not frontend::llvm_ir_translator::is_semicolon_or_empty_scope(body))
    generate_statement(body, context);
  else warning::useless_condition(node);
...
void
visit(
    Else const& node, llvmIrTranslatorContext& context
)
{
  auto&& body = node.body();
  if (not frontend::llvm_ir_translator::is_semicolon_or_empty_scope(body))
    generate_statement(body, context);
  else warning::useless_else(node);
}
...
void
visit(
    Condition const& node, llvmIrTranslatorContext& context
)
{
  auto&& current_func = context.current_block->getParent();
  ...
  auto&& if_blocks = std::vector<ConditionAndBody>{};
  if_blocks.reserve(ifs_size);

  auto&& if_condition_blocks = std::vector<llvm::BasicBlock*>{};
  if_condition_blocks.reserve(ifs_size);
  auto&& if_body_blocks = std::vector<llvm::BasicBlock*>{};
  if_body_blocks.reserve(ifs_size);
  ...
  auto&& else_block =
      node.has_else() ?
          llvm::BasicBlock::Create(context.context, "else", current_func) :
          nullptr;
  auto&& condition_end =
      llvm::BasicBlock::Create(context.context, "fi", current_func);
  ...
  else
  {
    for (auto &&it = 0lu, ite = ifs_size - 1; it != ite; ++it)
      generate_if_statement(
          ifs[it],
          context,
          if_blocks[it].condition,
          if_blocks[it].body,
          if_blocks[it + 1].condition,
          condition_end
      );
    ...
    generate_if_statement(
        ifs[ifs_size - 1],
        context,
        if_blocks[ifs_size - 1].condition,
        if_blocks[ifs_size - 1].body,
        else_block,
        condition_end
    );
  }
...
llvm::Value*
visit(
    Scope const& node, llvmIrTranslatorContext& context
)
{
  ...
  if (size == 0) throw error::using_scope_as_expression_but_its_empty(node);
  ...
  if (last.is_a<Return>()) generate_statement(last, context);
  else if (not last.supports<generatable_expression>())
    throw error::using_scope_as_expression_but_last_statement_isnt_expression(
        last
    );
  else return_value = generate_expression(last, context);
...
void
visit(
    Return const& node, llvmIrTranslatorContext& context
)
{
  auto&& not_in_function =
      (context.current_scope_status == ValueStatus::global);
...
llvm::Value*
visit(
    FunctionDeclaration const& node, llvmIrTranslatorContext& context
)
{
  auto&& args = node.args();
  ...
  auto&& args_types =
      std::vector<llvm::Type*>(args.size(), context.builder.getInt32Ty());
  /* all functions return int32 */
  auto&& type = llvm::FunctionType::get(
      std::move(return_type), std::move(args_types), false
  );
  /* force_declare function in ir with mangled name, cause function overload
   * must be supported */
  auto&& mangled_name = frontend::llvm_ir_translator::functions_table::
      FunctionsTable::mangle_name(node.name(), {args.begin(), args.end()});
  ...
  auto&& old_basic_block =
      static_cast<llvm::BasicBlock*>(context.current_block);
  auto&& entry_block = llvm::BasicBlock::Create(
      context.context, mangled_name + "-entry", function
  );
  context.set_current_block(entry_block);

  /* set  arguments name and put it in nametable */
  auto&& func_args = function->args();
  assert(std::distance(func_args.begin(), func_args.end()) == args.size());

  if (not node.name().empty()) context.functable.declare(node, function);
  ...
  if (body.empty()) throw error::function_with_empty_body(node);

  auto&& last_statement = body.back();

  if ((not last_statement.is_a<Return>()) and
      (not last_statement.supports<generatable_expression>()))
    throw error::
        last_function_statement_is_not_return_and_cannot_be_converted_to_expression(
            node, last_statement
        );
...
)
{
  ...
  std::ignore =
      visit<FunctionDeclaration, llvm::Value*, llvmIrTranslatorContext&>(
          node, context
      );
}
...
llvm::Value*
visit(
    FunctionCall const& node, llvmIrTranslatorContext& context
)
{
  auto&& args = node.args();
  auto&& name = node.name();
  ...
  auto&& value = context.nametable.get(name);
  if (not value) throw error::using_undeclarated_function(node);

  auto&& function = llvm::dyn_cast<llvm::Function>(value);

  if (not function)
    throw error::using_int_as_function(
        node, context.nametable.get_info(node.name()).declaration_node
    );

  if (function->arg_size() != completed_args.size())
    throw error::function_alias_arguments_mismatch(
        node, function->arg_size(), completed_args.size()
    );
...
)
{
  ...
  std::ignore = visit<FunctionCall, llvm::Value*, llvmIrTranslatorContext&>(
      node, context
  );
}
...
void
generate_llvm_ir(
    ast::AST const& ast, options::Options const& options
)
{
  ...
  auto&& main_function = llvm::Function::Create(
      main_type, llvm::Function::ExternalLinkage, "main", context.module
  );

  auto&& entry_block =
      llvm::BasicBlock::Create(context.context, "main-entry", main_function);
  ...
  auto&& out =
      llvm::ToolOutputFile{ir_file.string(), ec, llvm::sys::fs::OF_None};
