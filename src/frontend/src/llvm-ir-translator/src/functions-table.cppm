...
public:
  ...
  declaration_args_t(
      ast::node::FunctionDeclaration const& funcdecl
  ) :
      args_quantity_(funcdecl.args().size())
  {}

  declaration_args_t(
      call_args_t const& call_args
  ) :
...
bool
operator ==(
    declaration_args_t const& declaration_args, call_args_t const& call_args
)
{
  /* we can compare only sizes, cause we have only 1 type in language (int32) */
  return (call_args.size() == declaration_args.args_quantity());
}
...
template <>
struct hash<
    ParaCL::frontend::llvm_ir_translator::functions_table::declaration_args_t
>
{
  size_t
  operator ()(
...
namespace ParaCL::frontend::llvm_ir_translator::functions_table
{
export class FunctionsTable final
{
private:
...
private:
  function_info_t*
  lookup_(
      std::string_view name, declaration_args_t const& args
  )
  {
    auto&& overload_set = functions_.find(name);
    if (overload_set == functions_.end()) return nullptr;

    auto&& function = (overload_set->second).find(args);

    if (function == (overload_set->second).end()) return nullptr;
...
public:
  void
  check_unused_call_names() const noexcept
  {
    for (auto&& overload_set : functions_)
    {
      for (auto&& function : overload_set.second)
      {
        ...
        if (function_info.used or function_info.declaration_node.name().empty())
          continue;
...
public:
  ...
  std::string
  mangle_name(
      std::string_view base_name, declaration_args_t const& args
  )
  {
    if (not base_name.empty())
      return "__" +
             ...
             "_" +
             std::to_string(args.args_quantity());
...
public:
  void
  declare(
      ast::node::FunctionDeclaration const& funcdecl, llvm::Function* funcvalue
  )
  {
  ...
  llvm::Value*
  call(
      std::string_view name, call_args_t const& args
  )
  {
  ...
  )
  {
    ...
    for (auto&& overload_set : t.functions_)
    {
      ...
      {
        std::cout << "\t\t" << mangle_name(overload_set.first, func.first)
                  << std::endl;
      }
