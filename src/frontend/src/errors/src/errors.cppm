...
std::string
to_string(
    ast::node::CodeLocation const& location
)
{
  auto&& loc = std::ostringstream{};
  loc << location.file() << ":" << location.line_begin() << ":"
...
)
{
  ...
  // for (auto&& it = location.line_begin(), ite = location.line_end(); it !=
  // ite; ++it)
  // {
  auto&& string_begin_offset =
      location.column_begin() >= 1 ? location.column_begin() - 1 : 1;
  auto&& token_length =
      (location.line_begin() == location.line_end()) ?
  ...
  if (token_length > 1) mark << std::string(token_length - 1, '~');
...
std::string
show_error(
    std::string_view msg, ProblemStatus status = ProblemStatus::Error
)
{
  auto&& explain = std::ostringstream{};
  explain << BOLD "paracl: " RESET
          << ((status == ProblemStatus::Error) ? ERROR "error:" :
                                                 WARNING "warning:")
...
std::string
show_code_error(
    std::string_view msg, ast::node::CodeLocation const& location,
    ProblemStatus status = ProblemStatus::Error
)
{
  auto&& explain = std::ostringstream{};
  explain << BOLD << to_string(location) << ": " RESET
          << ((status == ProblemStatus::Error) ? ERROR "error:" :
                                                 WARNING "warning:")
          << " " RESET BOLD << msg << RESET "\n"
          << show_code(location);
...
std::string
was_declared_here(
    std::string_view name, ast::node::CodeLocation const& location,
    std::string user_msg = "was declared here: "
)
{
...
{
public:
  using_undeclarated_variable(
      ast::node::Variable const& node
  )
  {
    auto&& explain =
        "use of undeclared identifier '" + std::string(node.name()) + "'";
...
{
public:
  function_alias_arguments_mismatch(
      ast::node::FunctionCall const& node, size_t expected_args_size,
  ...
  )
  {
    auto&& explain = "function alias signature mismatch, '" +
                     ...
                     "' expects " +
                     std::to_string(expected_args_size) +
                     " arg" +
                     ((expected_args_size != 1) ? "s" : "") +
                     ", but called with " +
                     std::to_string(call_args_size) +
                     ":";
    msg_           = show_code_error(explain, node.location());
  }
};
...
{
public:
  using_int_as_function(
      ast::node::FunctionCall const& node,
      ast::node::Variable const&     declaration_node
  )
  {
    auto&& explain = "using '" +
                     std::string(node.name()) +
                     "' as function, but it has `int` type";
    msg_           = show_code_error(explain, node.location()) +
                     "\n" +
                     note(was_declared_here(
                         declaration_node, "was declared as integer here: "
                     ));
  }
};
...
public:
  ...
  )
  {
    auto&& explain = "using '" +
                     std::string(node.name()) +
                     "' as integer, but it has function type.";
    msg_           = show_code_error(explain, node.location()) +
                     "\n" +
                     note(was_declared_here(
                         declaration_node, "was declared as function here: "
                     ));
  }
};
...
{
public:
  using_undeclarated_function(
      ast::node::FunctionCall const& node
  )
  {
    auto&& explain =
        "call to undeclared function '" + std::string(node.name()) + "'";
    msg_ = show_code_error(explain, node.location());
  }
};
...
{
public:
  using_variable_from_parent_function_scope(
      ast::node::Variable const& node,
      ast::node::Variable const& declaration_node
  )
  {
    auto&& explain = "'" +
                     std::string(node.name()) +
                     "' was declared in parent scope, but isn`t global "
                     "variable (belong to the parent function scope)";
    msg_           = show_code_error(explain, node.location()) +
                     "\n" +
...
public:
  last_function_statement_is_not_return_and_cannot_be_converted_to_expression(
      ast::node::FunctionDeclaration const& node,
      ast::node::BasicNode const&           last
  )
  {
    auto&& explain = "in function '" +
                     std::string(node.name()) +
                     "':\n" RESET +
                     show_code(node.location()) +
                     "\n" BOLD
                     "last statement isn`t 'return' and cannot be converted to "
                     "expression" RESET;
    msg_           = show_code_error(explain, last.location());
  }
};

class using_scope_as_expression_but_last_statement_isnt_expression :
    public error
{
public:
  using_scope_as_expression_but_last_statement_isnt_expression(
      ast::node::BasicNode const& node
  )
  {
    auto&& explain =
        "last statement in scope isn`t expression, but scope used as "
        "expression";
    msg_ = show_code_error(explain, node.location());
  }
};
...
{
public:
  using_scope_as_expression_but_its_empty(
      ast::node::Scope const& node
  )
  {
...
{
public:
  redeclaration_of_function(
      ast::node::FunctionDeclaration const& node,
      ast::node::FunctionDeclaration const& declaration_node
  )
  {
...
{
public:
  return_in_not_function_scope(
      ast::node::Return const& node
  )
  {
...
public:
  function_arguments_with_same_names(
      ast::node::Variable const& /* FIXME: when show eror will shows more than 1
                                    location, add this argument*/
      ,
...
{
public:
  set_integer_variable_function_value(
      ast::node::Variable const& node,
      ast::node::Variable const& declaration_node
  )
  {
    auto&& explain = "set '" +
                     std::string(node.name()) +
                     "' function value, but it has integer type";
    msg_           = show_code_error(explain, node.location()) +
                     "\n" +
                     note(was_declared_here(declaration_node));
  }
...
{
public:
  set_function_variale_integer_value(
      ast::node::Variable const& node,
      ast::node::Variable const& declaration_node
  )
  {
    auto&& explain = "set '" +
                     std::string(node.name()) +
                     "' integer value, but it has function type";
    msg_           = show_code_error(explain, node.location()) +
                     "\n" +
                     note(was_declared_here(declaration_node));
  }
...
std::ostream&
function_return_value_ignored(
    ast::node::FunctionCall const& node, std::ostream& os = std::cerr
)
{
  auto&& explain =
      "return value of '" + std::string(node.name()) + "' is ignored";
...
expression_result_unused(
    ast::node::CodeLocation const& location, std::ostream& os = std::cerr
)
{
...
std::ostream&
unnamed_function(
    ast::node::FunctionDeclaration const& node, std::ostream& os = std::cerr
)
{
...
std::ostream&
empty_scope(
    ast::node::Scope const& node, std::ostream& os = std::cerr
)
{
...
std::ostream&
useless_semicolon(
    ast::node::Semicolon const& node, std::ostream& os = std::cerr
)
{
...
std::ostream&
useless_condition(
    ast::node::If const& node, std::ostream& os = std::cerr
)
{
...
std::ostream&
useless_else(
    ast::node::Else const& node, std::ostream& os = std::cerr
)
{
...
std::ostream&
unused_variable(
    ast::node::Variable const& node, std::ostream& os = std::cerr
)
{
...
std::ostream&
unused_function_call_name(
    ast::node::FunctionDeclaration const& node, std::ostream& os = std::cerr
)
{
  auto&& explain =
      "useless function call name '" + std::string(node.name()) + "'";
  os << show_code_error(explain, node.location(), ProblemStatus::Warning)
     << "\n";
