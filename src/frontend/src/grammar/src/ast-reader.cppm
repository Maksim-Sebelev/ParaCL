...
)
{
  ...
  auto CFileCloser = [](std::FILE* fptr)
  {
    assert(fptr && "std::fclose(nullptr)");
    std::fclose(fptr);
  };
  using CFilePtrRaiiWrapper = std::unique_ptr<std::FILE, decltype(CFileCloser)>;

  auto&& input_file_wrapper =
      CFilePtrRaiiWrapper{std::fopen(input_file.c_str(), "r")};
  auto&& input_file_ptr = input_file_wrapper.get();

  if (not input_file_ptr) throw error::no_such_file_error(input_file);
