...
)
{
  ...
  while (current_line <
         loc.begin.line &&
         std::fgets(buffer, sizeof(buffer), in))
  {
    ++current_line;
  }
...
export
frontend::ast::node::CodeLocation
location_cast(
    yy::location const& loc, FILE* in
)
{
  return frontend::ast::node::CodeLocation{
    current_file,
    static_cast<frontend::ast::node::CodeLocation::code_place_uint_t>(
        loc.begin.line
    ),
    static_cast<frontend::ast::node::CodeLocation::code_place_uint_t>(
        loc.end.line
